...
bezpośrednio wykorzystywane są inne klasy wchodzące do projektu
...
Z testowania czarno\dywiz skrzynkowego dodatkowo wyłączone zostaną grupy klas.
\begin{itemize}
    \item Klasy niewykorzystywane ze względu na wybór implementacji:
        \begin{itemize}
            \item \texttt{DFS\_SearchList},
            \item \texttt{FlowNetworkAdjacencyList},
            \item \texttt{ShortestPathArray}.
        \end{itemize}
    \item Klasy abstrakcyjne, po których dziedziczą wykorzystywane przy testowaniu klasy:
        \begin{itemize}
            \item \texttt{FlowNetwork}
            \item \texttt{Search}
        \end{itemize}
    \item Klasy szczegółowe, wykorzystywane wewnątrz implementacji:
        \begin{itemize}
            \item \texttt{EdgeInfo},
            \item \texttt{DFS\_SearchArray} -- implementująca algorytm
                przeszukiwania \textsc{Depth\dywiz first search} na
                implementacji sieci z klasy \texttt{FlowNetworkArray}, której
                instancja jest wymagana jako drugi argument konstruktura klasy
                \texttt{FordFulkerson};
...
\begin{enumerate}
    ...
    \item Ze względu pętle:
        \begin{enumerate}
            ...
            \item Bez pętli.
        \end{enumerate}
    \item Ze względu na krawędzie bezpośrednich między źródłem, a ujściem:
        \begin{enumerate}
