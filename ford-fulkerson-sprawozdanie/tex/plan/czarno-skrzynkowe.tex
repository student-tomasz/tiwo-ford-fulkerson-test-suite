...
Wyłączenia z testowania funkcjonalności obejmują wyłączenia projektu, a dodatkowo, ze względu na wybór implementacji:
\begin{itemize}[nosep]

\end{itemize}

klasy abstrakcyjne, po których dziedziczą wykorzystywane klasy:
\begin{itemize}[nosep]

\end{itemize}

szczegóły wykorzystywanej implementacji:
\begin{itemize}[nosep]

\end{itemize}

\begin{itemize}[nosep]
    \item Klasa \texttt{DFS\_SearchList} --- ze względu na wybór implementacji opartej o macierze
    \item Klasa \texttt{FlowNetworkAdjacencyList} --- ze względu na wybór implementacji opartej o macierze
    \item Klasa \texttt{EdgeInfo} --- jest wykorzystywana, ale stanowi szczegół implementacji grafu przepływu
...
%Do Kierownika: Możesz pomyśleć nad dodaniem Example.java, do project-wide wyłączeń z testów
    \item Klasa abstrakcyjna \texttt{FlowNetwork} --- wykorzystujemy rozszerzającą klasę \texttt{FlowNetworkArray}
    \item Klasa \texttt{FlowNetworkArray} --- implementująca sieć przepływową w oparciu o macierz, wykorzystywana do przechowywania wejściowej sieci przepływowej
    ...
    \item Klasa abstrakcyjna\texttt{Search} --- wykorzystujemy rozszerzającą klasę \texttt{DFS\_SearchArray}
    \item Klasa \texttt{ShortestPathArray} --- ze względu na wybór implementacji opartej o klasę \texttt{DFS\_SearchArray}
    \item Klasa \texttt{VertexInfo} --- jest wykorzystywana, ale stanowi szczegół implementacji grafu przepływu
    \item Klasa \texttt{VertexStructure} --- jest wykorzystywana, ale stanowi szczegół implementacji grafu przepływu
\end{itemize}
